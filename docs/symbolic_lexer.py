...
class SymbolicLexer(RegexLexer):
    ...
    tokens = {
        'root': [
            ...
            (r'(?<![a-zA-Z_])(?:(?:\d{,3}(?:_?\d{3})+|\d+)(?<![\.\d])(?:\.\d*(?:[eE][+-]?\d+)?)|(?:\d{,3}(?:_?\d{3})+|\d+)(?<![\.\d])(?:(?:\.\d+)?[eE][+-]?\d+))', Number.Float),  # Float with optional exponent and underscores
            (r'(?<![a-zA-Z_])0[oO][0-7_]+\b', Number.Oct),            # Octal (e.g., 0o755, 0o7_55)
            (r'(?<![a-zA-Z_])0[bB][01_]+\b', Number.Bin),             # Binary (e.g., 0b1010, 0b1_010)
            (r'(?<![a-zA-Z_])0[xX][0-9a-fA-F_]+\b', Number.Hex),      # Hexadecimal (e.g., 0x1F4, 0x_FF_FF)
